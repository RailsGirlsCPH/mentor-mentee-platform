RSpec.describe Wish, type: :model do
  ...
  describe 'validations' do
    ...
    it 'Custom Validation ensure user available offline or online' do
      wish1 = Wish.new(api_user_id: api_user.id,
                       programminglanguage_id: prog.id,
                       meetinginterval_id: meet.id,
                       available_offline: false,
                       available_online: false)
      expect(wish1).to be_invalid
      expect(wish1.errors.full_messages).to include(/Api user must be available either online\
 or offline to complete wish/)
    end
    it 'Custom Validation ensure if you are a mentee before you make an wish' do
      wish2 = Wish.new(api_user_id: api_user.id,
                       programminglanguage_id: prog.id,
                       meetinginterval_id: meet.id,
                       available_offline: false,
                       available_online: false)
