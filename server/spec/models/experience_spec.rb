  ...
  describe 'validations' do
    subject do
      create(:experience,
             ...
             programminglanguage_id: prog.id,
             meetinginterval_id: meet.id)
    end
    ...
    it 'Custom Validation ensure user available offline or online' do
      experience1 = Experience.new(api_user_id: api_user.id,
                                   programminglanguage_id: prog.id,
                                   meetinginterval_id: meet.id,
                                   available_offline: false,
                                   available_online: false)
      expect(experience1).to be_invalid
      expect(experience1.errors.full_messages).to include(/Api user must be available \
either online or offline to complete experience/)
    end
    it 'Custom Validation ensure if you are a mentor before you make an experience' do
      experience2 = Experience.new(api_user_id: api_user.id,
                                   programminglanguage_id: prog.id,
                                   meetinginterval_id: meet.id,
                                   available_offline: false,
                                   available_online: false)
      ...
      expect(experience2.errors.full_messages).to include(/Api user if you are creating\
 an experience you must be a mentor/)
    end
  end
end
