class Api::V1::ApiUsersController < ApplicationController
  ...
  #GET /api_users
  def index
    @api_users = ApiUser.includes(:wishes).includes(wishes: :programminglanguage).includes(wishes: :meetinginterval).includes(:experiences).includes(experiences: :programminglanguage).includes(experiences: :meetinginterval).all
    ...
    #@api_users = @api_users.as_json(only: [:wishes]) if params[:wishes]
    #puts "lala" if CGI.parse(URI.parse(request.url).query).keys[0] == 'wishes'
    #@api_users = @api_users.(wishes) if CGI.parse(URI.parse(request.url).query).keys[0] == 'wishes'
    ##binding.pry
  end
