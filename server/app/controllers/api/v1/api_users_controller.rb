class Api::V1::ApiUsersController < ApplicationController
  ...
  # GET /api_users
  def index
    @api_users = ApiUser.includes(wishes: %i[programminglanguage meetinginterval],
                                  experiences: %i[programminglanguage meetinginterval]).all
    # Explanation regarding includes: not necessary to link programming languages and
    # meeting interval to ApiUser, but it means there is only one call to the database
  ...
  def api_user_params
    params.permit(:first_name, :last_name, :city, :email, :username, :mentor, :mentee, :password_digest, :password,
