...
class Api::V1::WishesController < ApplicationController
  ...
  def index
    @wishes = @api_user.wishes.includes(:programminglanguage).includes(:meetinginterval)  if params[:api_user_id]
    #@wishes = Wish.includes(:programminglanguage).includes(:meetinginterval).all if params[:wishes]
    ...
    #@wishes = @wishes.where(programminglanguage: "SCALA") if params[:language] ==
    #puts CGI.parse(URI.parse(request.url).query).values[0][0]
  ...
  def wish_params
    ...
    # params.permit(:available_offline, :available_online, :goal, :api_user_id, :programminglanguage_id, :meetinginterval_id)
  end
  ...
  # def set_wish
  #   @wish = Wish.includes(:programminglanguage).includes(:meetinginterval).find(params[:id])
  # end
